...
export function ASTNodeView({ node, depth, onSelect, selectedRange }: Props) {
  ...
  return (
    <div className={`flex flex-col ${depth > 0 ? "ml-3 lg:ml-4 border-l border-slate-100" : ""}`}>
      <div
        className={`flex items-center gap-1.5 lg:gap-2 px-1.5 lg:px-2 py-1.5 lg:py-1 cursor-pointer transition-all rounded-md group m-0.5 ${
          isSelected 
            ? "bg-blue-500/10 ring-1 ring-blue-500/30 shadow-sm" 
            : "hover:bg-slate-100"
        }`}
        onClick={handleSelect}
      >
        <span 
          className={`text-[9px] lg:text-[10px] w-4 h-4 flex justify-center items-center transition-transform duration-200 ${hasChildren ? "opacity-30 group-hover:opacity-70" : "opacity-0"} ${isOpen ? "rotate-0" : "-rotate-90"}`}
          onClick={toggleOpen}
        >
          ▼
        </span>
        <span className={`font-mono text-[12px] lg:text-[13px] ${isSelected ? "text-blue-600 font-bold" : "text-slate-600"}`}>
          {node.type}
        </span>
